...
{    
    ...
    ESP_ERROR_CHECK(i2c_mcp23017_start(0x20, 1000)); // Dirección I2C=0x20, timeout=1000ms
    ...
    while(1)
    {
        ...
        if (err != ESP_OK) {
            ...
            vTaskDelay(pdMS_TO_TICKS(2500)); // Wait before retrying
            continue;
        }
        ...
        {
            ESP_LOGI(TAG, "GPIOA inputs: 0x%02X (A0..3: %d, %d, %d, %d; A4..7: %d, %d, %d, %d)",
                     gpioa, (gpioa & 0x01) ? 1 : 0, (gpioa & 0x02) ? 1 : 0, (gpioa & 0x04) ? 1 : 0, (gpioa & 0x08) ? 1 : 0,
                     (gpioa & 0x10) ? 1 : 0, (gpioa & 0x20) ? 1 : 0, (gpioa & 0x40) ? 1 : 0, (gpioa & 0x80) ? 1 : 0);
        }
        ...
        if (err != ESP_OK) {
            ...
            ESP_LOGI(TAG, "GPIOB inputs: 0x%02X (B0..3: %d, %d, %d, %d; B4..5: %d, %d; B6..7: %d, %d)",
                     gpiob, (gpiob & 0x01) ? 1 : 0, (gpiob & 0x02) ? 1 : 0, (gpiob & 0x04) ? 1 : 0, (gpiob & 0x08) ? 1 : 0,
                     (gpiob & 0x10) ? 1 : 0, (gpiob & 0x20) ? 1 : 0, (gpiob & 0x40) ? 1 : 0, (gpiob & 0x80) ? 1 : 0);
        }
